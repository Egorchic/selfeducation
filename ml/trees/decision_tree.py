...
class DecisionTree:
    ...
    @staticmethod
    def _weightedEntropy(L, R: np.ndarray) -> float:
        return len(L) / (len(L) + len(R)) * DecisionTree.H(L) + len(R) / (len(L) + len(R)) * DecisionTree.H(R)

    @staticmethod
    def _findBestParams(X, y) -> tuple:
        ...
        for feature_i in range(X.shape[1]):
            ...
            for i in range(len(features) - 1):
                ...
                information = DecisionTree._weightedEntropy(left, right)
    ...
    def buildTree(self, X, y: np.ndarray, current_node: Node):
        ...
        best_params = DecisionTree._findBestParams(X, y)
